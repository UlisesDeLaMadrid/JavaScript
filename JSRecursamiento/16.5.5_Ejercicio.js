...
const genres = (movieArr) => {
    return movieArr.reduce((accum,current) => {
        if (!accum.includes(current.genre)) {
            return [...accum, current.genre]
        }
        return accum
    }, [])
}
...
const directors = (movieArr) => {
    return movieArr.reduce((accum,current) => {
        if (!accum.includes(current.director)) {
            return [...accum, current.director]
        }
        return accum
    }, [])
}
...
const protagonists = (movieArr) => {
    return movieArr.reduce((accum, current) => {
        current.protagonists.forEach(protagonist => {
            if (!accum.includes(protagonist.name)) {
                accum = [...accum, protagonist.name];
            }
        });
        return accum;
    }, []);
...
const ratings = (movieArr) => {
    return movieArr.reduce((accum,current) => {
        if (!accum.includes(current.rating)) {
...
/*5. Obtener una lista de las películas que tienen una duración entre 2 rangos, por ejemplo
las películas que duran entre 100 y 120 mins ( los valores de las duraciones deben ser dinámicos)*/

const duration = (movieArr, minDuration, maxDuration) => {
...
console.log(duration(movies, 100, 120));
